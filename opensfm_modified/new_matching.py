...
from opensfm_modified import new_feature_loader
...
def clear_cache():
    new_feature_loader.instance.clear_cache()
...
def save_matches(file_path, images_ref, matched_pairs):
...
def match_unwrap_args(args):
    ...
    im1_matches = {}
    p1, f1, _ = new_feature_loader.instance.load_points_features_colors(ctx.opensfm_config, im1, ctx.feature_path, ctx.file_path+im1)
    camera1 = ctx.cameras[ctx.exifs[im1]['camera']]

    for im2 in candidates:
        p2, f2, _ = new_feature_loader.instance.load_points_features_colors(ctx.opensfm_config, im2, ctx.feature_path, ctx.file_path+im2)
        camera2 = ctx.cameras[ctx.exifs[im2]['camera']]

        im1_matches[im2] = match(im1, im2, camera1, camera2, ctx.opensfm_config, ctx.feature_path, ctx.file_path+im2)
...
def match(im1, im2, camera1, camera2, data, feature_path, image_path):
    """Perform matching for a pair of images."""
    # Apply mask to features if any
    time_start = timer()
    p1, f1, _ = new_feature_loader.instance.load_points_features_colors(
        data, im1,feature_path, image_path  ,masked=True)
    p2, f2, _ = new_feature_loader.instance.load_points_features_colors(
        data, im2,feature_path,image_path ,masked=True)
    ...
    if matcher_type == 'WORDS':
        w1 = new_feature_loader.instance.load_words(feature_path, data, im1, masked=True)
        w2 = new_feature_loader.instance.load_words(feature_path, data, im2, masked=True)
        ...
        i1 = new_feature_loader.instance.load_features_index(data, im1, feature_path ,masked=True)
        if symmetric_matching:
            i2 = new_feature_loader.instance.load_features_index(data, im2, feature_path ,masked=True)
    ...
    m1 = new_feature_loader.instance.load_mask(feature_path,data, im1,image_path)
    m2 = new_feature_loader.instance.load_mask(feature_path, data, im2, image_path)
